...
import com.hkmonumeena.ecart.fragments.AllProductsFragment
...
class RvAllProducts(items: ArrayList<Product>, val allProducts: AllProductsFragment):GenericAdapter<Product>(items) {
    override fun configure(item: Product, holder: ViewHolder, position: Int) {
        with(holder.itemView){
           ...
           setOnClickListener {
               allProducts.multipleStackNavigator?.start(ProductDetailsFragment(item))
           }
        }
    }
